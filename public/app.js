...
const youtubeLiveDot = $("#youtubeLiveDot");
...
youtubeSeek.addEventListener("input", updateYoutubeSeekPreview);
youtubeSeek.addEventListener("change", seekYoutubeVideo);
youtubeSeek.addEventListener("pointerdown", () => {
  state.youtubeSeeking = true;
});
youtubeSeek.addEventListener("pointerup", () => {
...
document.addEventListener("fullscreenchange", updateYoutubeFullscreenButton);
...
async function handleSignal(message) {
  ...
  if (message.type === "youtube-state") {
    await applyYoutubeState(message);
    return;
  }
  ...
  if (message.type === "screen-share") {
    ...
    if (message.from !== state.id) {
      if (message.enabled === true) {
        requestScreenRelay(message.from, true);
        setTimeout(() => {
  ...
  if (message.type === "joined") {
    ...
    if (SCREEN_SHARING_ENABLED && state.screenSharing && state.screenTrack?.readyState === "live") {
...
function updateOnlineUser(user, updateCounter = true) {
  ...
  if (user.id !== state.id && user.screenSharing) {
    requestScreenRelay(user.id, true);
  }
}
  ...
  try {
    ...
    state.youtubePlayer = new YT.Player("youtubePlayer", {
      ...
      events: {
        ...
        onError: () => showToast("this YouTube video cannot be played here")
      }
    });
    return state.youtubePlayer;
  } catch (error) {
    showToast(error.message || "YouTube player could not load");
    return null;
  }
}

async function toggleYoutubePanel() {
  if (!youtubePanel.hidden) {
    hideYoutubePanel();
    return;
  }
  await showYoutubePanel();
}

async function showYoutubePanel() {
  youtubePanel.hidden = false;
  youtubeButton.setAttribute("aria-pressed", "true");
...
function destroyYoutubePlayer() {
  clearInterval(state.youtubeUiTimer);
  state.youtubeUiTimer = null;
  ...
  directMediaPlayer.removeAttribute("src");
  directMediaPlayer.load();
  directMediaPlayer.hidden = true;
  if (!document.getElementById("youtubePlayer")) {
    const mount = document.createElement("div");
    mount.id = "youtubePlayer";
    youtubeStage.prepend(mount);
  }
}

function queueYoutubeVideo(event) {
  event.preventDefault();
...
async function applyYoutubeState(message) {
  ...
  state.youtube = {
    ...
    currentId: String(message.currentId || ""),
    kind: message.kind === "direct" ? "direct" : message.videoId ? "youtube" : "",
...
function renderYoutubeRoom() {
  const active = Boolean(state.youtube.currentId);
  youtubeLiveDot.hidden = !active;
  youtubeStage.hidden = !active;
  ...
  state.youtube.queue.forEach((item, index) => {
    const row = document.createElement("div");
    row.className = `youtube-queue-item${index === state.youtube.currentIndex ? " current" : ""}`;
    const number = document.createElement("div");
    number.className = "youtube-queue-number";
    number.textContent = String(index + 1);
    const info = document.createElement("div");
    info.className = "youtube-queue-info";
    const title = document.createElement("div");
    title.className = "youtube-queue-title";
    const kind = document.createElement("span");
    kind.className = "youtube-queue-kind";
    kind.textContent = item.kind === "direct" ? item.mediaType || "media" : "youtube";
    const label = item.kind === "direct"
      ? item.title || item.url
      : state.youtubeTitles.get(item.videoId) || item.title || `YouTube video ${item.videoId}`;
    title.append(kind, document.createTextNode(label));
    const meta = document.createElement("div");
    meta.className = "youtube-queue-meta";
    meta.textContent = `queued by ${item.addedByName || item.addedBy}`;
    info.append(title, meta);
    row.append(number, info);
    if (state.inCall) {
      const remove = document.createElement("button");
      remove.className = "small youtube-remove";
      remove.type = "button";
      remove.dataset.id = item.id;
      const voters = Array.isArray(item.removeVotes) ? item.removeVotes : [];
      const required = Math.max(1, Number(item.removeRequired) || 1);
      const voted = voters.includes(state.username);
      const queuedByMe = item.addedBy === state.username;
      remove.textContent = queuedByMe
        ? "remove"
        : voted ? `voted ${voters.length}/${required}` : `remove ${voters.length}/${required}`;
      remove.disabled = !queuedByMe && voted;
      row.append(remove);
    }
    youtubeQueue.append(row);
  });
  updateYoutubeTimeline();
}

function isYoutubeController() {
  return Boolean(state.username && state.youtube.controller === state.username);
}
...
function showYoutubePlayer() {
  if (!directMediaPlayer.paused) directMediaPlayer.pause();
  const mount = document.getElementById("youtubePlayer");
  if (mount) mount.hidden = false;
  directMediaPlayer.hidden = true;
}

function showDirectMediaPlayer() {
  state.youtubePlayer?.pauseVideo?.();
  const mount = document.getElementById("youtubePlayer");
  if (mount) mount.hidden = true;
  directMediaPlayer.hidden = false;
}

function reconcileDirectMedia(force = false) {
  showDirectMediaPlayer();
...
function handleDirectMediaLoaded() {
  if (state.youtube.kind !== "direct") return;
  const expected = youtubeExpectedPosition();
  if (Number.isFinite(directMediaPlayer.duration)) {
    directMediaPlayer.currentTime = Math.min(expected, directMediaPlayer.duration);
  }
  if (state.youtube.status === "playing") directMediaPlayer.play().catch(() => {});
  updateYoutubeTimeline();
}
...
function handleDirectMediaPause() {
  if (
    state.youtube.kind !== "direct"
    || state.directMediaApplying
    || directMediaPlayer.ended
    || !isYoutubeController()
  ) return;
  const position = directMediaPlayer.currentTime || 0;
...
function updateYoutubeVideoTitle() {
  ...
  state.youtubeTitles.set(data.video_id, data.title);
  renderYoutubeRoom();
}

function toggleYoutubePlayback() {
  if (!isYoutubeController() || !state.youtube.currentId) return;
  const position = state.youtube.kind === "direct"
    ? Number(directMediaPlayer.currentTime || youtubeExpectedPosition())
    : Number(state.youtubePlayer?.getCurrentTime?.() || youtubeExpectedPosition());
  ...
  if (state.youtube.kind === "direct") {
    ...
    else directMediaPlayer.play().catch(() => {});
    setTimeout(() => {
      state.directMediaApplying = false;
    }, 500);
  } else if (playing) {
...
function seekYoutubeVideo() {
  ...
  if (state.youtube.kind === "direct") {
    ...
    if (playing) directMediaPlayer.play().catch(() => {});
    setTimeout(() => {
      state.directMediaApplying = false;
    }, 500);
  } else {
...
function updateYoutubeSeekPreview() {
  if (!isYoutubeController()) return;
  youtubeTime.textContent = `${formatYoutubeTime(Number(youtubeSeek.value) || 0)} / ${formatYoutubeTime(youtubeDuration())}`;
}
...
function removeYoutubeQueueItem(event) {
  const button = event.target.closest(".youtube-remove");
  if (!button?.dataset.id) return;
  send({ type: "youtube-queue-remove", id: button.dataset.id });
}

function youtubeDuration() {
  return state.youtube.kind === "direct"
    ? Math.max(0, Number(directMediaPlayer.duration || 0))
    : Math.max(0, Number(state.youtubePlayer?.getDuration?.() || 0));
}

function currentSharedMediaTime() {
  return state.youtube.kind === "direct"
    ? Math.max(0, Number(directMediaPlayer.currentTime || 0))
    : Math.max(0, Number(state.youtubePlayer?.getCurrentTime?.() || 0));
}

function updateYoutubeTimeline() {
  const duration = youtubeDuration();
  const current = state.youtube.kind === "direct"
    ? Math.max(0, Number(directMediaPlayer.currentTime || youtubeExpectedPosition()))
    : state.youtubeReady
      ? Math.max(0, Number(state.youtubePlayer?.getCurrentTime?.() || youtubeExpectedPosition()))
      : youtubeExpectedPosition();
  youtubeSeek.max = String(Math.max(1, duration));
  if (!state.youtubeSeeking && document.activeElement !== youtubeSeek) {
    youtubeSeek.value = String(Math.min(current, Math.max(1, duration)));
  }
  youtubeTime.textContent = `${formatYoutubeTime(current)} / ${formatYoutubeTime(duration)}`;
}
...
function startYoutubeTimers() {
  if (!state.youtubeUiTimer) {
    state.youtubeUiTimer = setInterval(() => {
...
function updateScreenShareButton() {
  ...
  shareScreenButton.hidden = !SCREEN_SHARING_ENABLED || !pc;
...
async function toggleScreenShare() {
  if (!SCREEN_SHARING_ENABLED) return;
...
async function startScreenRelayProducer() {
  ...
  const produce = async () => {
    ...
    try {
      ...
      // A single draw, encode, or send failure must not stop the relay loop.
    } finally {
      if (
        generation === state.screenRelayGeneration
        && state.screenSharing
...
function monitorRemoteScreen(id) {
  ...
  const inspect = () => {
    ...
    if (!relayActive) requestScreenRelay(id);
...
function setScreenShareCard(id, enabled, stream = null) {
  ...
  if (!SCREEN_SHARING_ENABLED) {
    view.hidden = true;
    card.dataset.screenSharing = "false";
  ...
  try {
    ...
    if (registration?.showNotification) {
      await registration.showNotification(title, {
        body,
        tag: options.tag || "healthpack-call",
        renotify: true,
        silent: false,
        icon: "/icon.svg",
        badge: "/icon.svg",
...
async function enablePushNotifications(showTest = false) {
  ...
  try {
    ...
    if (!subscribeResponse.ok) throw new Error("subscription save failed");
    state.notificationEnabled = true;
    state.notificationUnsupportedReason = "";
...
function supportsCallNotifications() {
  state.notificationUnsupportedReason = "";
  if (!window.isSecureContext) {
    state.notificationUnsupportedReason = "notifications need HTTPS";
    return false;
  }
  if (!("Notification" in window)) {
    state.notificationUnsupportedReason = "this browser does not support notifications";
    return false;
  }
  if (!("serviceWorker" in navigator) || !("PushManager" in window)) {
    state.notificationUnsupportedReason = "this browser cannot receive call alerts in the background";
    return false;
  }
  return true;
}
